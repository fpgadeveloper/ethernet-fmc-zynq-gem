...
PetaLinux. It can be built for the Ethernet FMC Zynq GEM reference designs with the cross-platform
`build.py` runner at the root of the repository, and produces a Linux image that exercises the
Ethernet FMC ports in exactly the same way as the PetaLinux flow.
...
has PetaLinux support). These designs connect the Ethernet FMC PHYs to the hard GEMs of the Zynq /
Zynq UltraScale+ PS through the PL GMII-to-RGMII IP.
...
## Using the Ethernet FMC ports

Once Linux has booted and you have logged in at the console, the Ethernet FMC ports are exercised
...
```{note}
**Interface names follow the systemd predictable-naming scheme.** As in the PetaLinux flow,
the PS-GEM Ethernet FMC ports appear as `end0`–`end3` on Zynq UltraScale+ targets and as
`enx<mac>` (MAC-derived) on Zynq-7000 targets. The interface number does **not** necessarily track
the FMC port number; identify a port by its MAC address (Ethernet FMC Port 0 = `00:0a:35:00:01:22`,
Port 1 = `…:23`, Port 2 = `…:24`, Port 3 = `…:25`). See the
[Port configurations](petalinux.md#port-configurations) table for the per-board port-to-interface
mapping, and substitute the appropriate name into the commands in
[Example Usage](petalinux.md#example-usage).
```
...
* **PS-GEM / external-PHY wiring (`port-config.dtsi`).** The external Ethernet-FMC PHYs are not
  described by the XSA, so each target applies a port-config overlay (`ports-0123` for four-port
  designs, `ports-012-` for the three-port `zcu102_hpc1`) that adds, for each active GEM, the MAC
  address, PHY handle, MDIO bus and GMII-to-RGMII converter register.
* **On-board PS Ethernet disabled (Zynq-7000).** `gem0` is disabled in `system-user.dtsi`; the
  on-board RJ45 is wired to `gem0`, which the 2025.x U-Boot data-aborts probing because `pcw.dtsi`
  leaves it without a PHY handle.
* **UART console pinning (Zynq UltraScale+).** The 2025.2 flow leaves `port-number = <0>` on both
  PS UARTs, so `system-user.dtsi` pins the port numbers and serial aliases to keep the console
  (cabled to UART0) deterministic at `ttyPS0`.
* **PHY drivers and GMII-to-RGMII shim.** Every board's `bsp.cfg` enables
  `CONFIG_XILINX_GMII2RGMII` and the Marvell / AMD / Xilinx PHY drivers the Ethernet FMC ports
  use.
